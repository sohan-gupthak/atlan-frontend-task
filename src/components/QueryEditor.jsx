...
const QueryEditor = ({ query, onQueryChange, onExecute }) => {
	...
	const formatQuery = () => {
		...
		let formatted = query
			.replace(
				/\b(SELECT|FROM|WHERE|GROUP BY|ORDER BY|HAVING|JOIN|LEFT JOIN|RIGHT JOIN|INNER JOIN|OUTER JOIN|LIMIT)\b/gi,
				'\n$1',
			)
			.replace(/\n/g, '\n  ')
			.trim();
	...
	const handleSaveQuery = () => {
		...
		const newSavedQueries = [...savedQueries, { id: Date.now(), name: queryName, query: query }];
	...
	return (
		<Card className="query-editor-card mb-3">
			<Card.Header>
				<div className="d-flex flex-column flex-md-row justify-content-between align-items-md-center">
					...
					<div className="d-flex flex-wrap">
						...
						<Button variant="primary" size="sm" onClick={handleExecute} title="Execute Query (Ctrl+Enter)">
			...
			<Card.Footer className="text-muted small">Press Ctrl+Enter to execute query</Card.Footer>
					...
					<Form>
						<Form.Group>
							...
							<InputGroup>
								...
								/>
								<Button variant="primary" onClick={handleSaveQuery} disabled={!queryName.trim()}>
									<FaCheck /> Save
								</Button>
							</InputGroup>
						</Form.Group>
