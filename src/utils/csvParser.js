	...
	try {
		...
		switch (tableName) {
			...
			case 'reviews':
				...
				for (let i = 1; i <= 20; i++) {
					baseReviewsData.push({
						id: i,
						product_id: `P${(i % 5) + 1}`,
						user_id: `U${(i % 10) + 1}`,
						profile_name: `User ${(i % 10) + 1}`,
						helpfulness_numerator: Math.floor(Math.random() * 50),
						helpfulness_denominator: Math.floor(Math.random() * 100) + 50,
						score: Math.floor(Math.random() * 5) + 1,
						time: new Date(Date.now() - Math.floor(Math.random() * 10000000000)).toISOString(),
						summary: `Review summary ${i}`,
						text: `This is a detailed review text for product ${(i % 5) + 1}. It contains the user's thoughts and opinions about the product.`,
					});
				}

				data = generateMoreRows(baseReviewsData, 980);

				console.log(`Generated 1000 mock rows for reviews table (simulating a large dataset)`);
				break;
			default:
...
export const loadMoreData = async (tableName, page = 1, pageSize = 1000, dataPath = '/') => {
	...
	try {
		const { data: fullData, columns } = await parseCSVData('reviews', dataPath);

		const totalRows = 3600000;
		const totalPages = Math.ceil(totalRows / pageSize);

		const startIndex = ((page - 1) % 10) * 100;
		const endIndex = startIndex + pageSize;
		const pageData = fullData.slice(startIndex % fullData.length, endIndex % fullData.length || fullData.length);
